...
def worker_function(i, noise, stress, x, du, dy_type):
    ...
    u_crit, binary = 0, 0
    temp = generate_sample_matrix(simulate(0.7 + i * du, noise, stress, x, dy_type))
...
if __name__ == "__main__":
    start_time = time.time()
    filenames = ['dolphins-s-u-homo-down', 'dolphins-s-u-hetero-down']
    print(filenames)
    count = 1
    stress, sigma, x, du, us, dD, Ds = init_conditions(1, 4)
    x_equis, var, u_crit, C_ensemble = equilibrium(sigma, stress, x, k, du, 4)
